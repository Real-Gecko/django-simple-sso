...
class AuthorizeView(View):
    ...
    def success(self):
        ...
        self.token.session = Session.objects.get(pk=self.request.session.session_key)
        self.token.save()
...
class Server:
    ...
    token_verify_timeout = datetime.timedelta(seconds=settings.SSO_TOKEN_VERIFY_TIMEOUT)
    ...
    def get_urls(self):
        return [
            ...
            re_path(
                r"^logout/$",
                provider_for_django(self.logout_provider(server=self)),
                name="simple-sso-logout",
            ),
        ]
